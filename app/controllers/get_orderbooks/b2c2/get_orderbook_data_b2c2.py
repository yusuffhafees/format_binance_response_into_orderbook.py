    ...
    async with websockets.connect('wss://otcapp-qa.tradias.de/otc/ws', extra_headers=headers) as websocket:
        ...
        while True:
            ...
            if 'event' in received_msg.keys():
                ...
                return format_b2c2_message(received_msg, instrument)
                # return Orderbook


def format_b2c2_message(b2c2_response, instrument):
    timestamp = datetime.datetime.strptime(b2c2_response['timestamp'], '%Y-%m-%dT%H:%M:%S.%fZ')  # unix time
    prices_for_ask = [d['price'] for d in b2c2_response['levels']['sell']]

    quantities_for_ask = [d['quantity'] for d in b2c2_response['levels']['sell']]
    ...
    prices_for_bid = [d['price'] for d in b2c2_response['levels']['buy']]
    quantities_for_bid = [d['quantity'] for d in b2c2_response['levels']['buy']]
...
if __name__ == "__main__":
    ...
    b2c2_instruments = df[df.xchg_code == 'B2C2']['curr_pair'].unique().tolist()
    orderbooks = []
    for instrument in b2c2_instruments:
